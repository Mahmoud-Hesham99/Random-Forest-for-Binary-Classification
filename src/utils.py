...
def load_and_split_data(
        file_dir_path: str, val_pct: float
        ) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Reads a CSV file in the given directory path as a pandas dataframe.
    Splits the loaded data into training and validation sets based on the
    given percentage.
    ...
    Returns:
        Tuple[pd.DataFrame, pd.DataFrame]: A tuple containing the training and
            validation sets as DataFrames.
    """
    data = read_csv_in_directory(file_dir_path)
    train_data, val_data = train_test_split(data, test_size=val_pct, random_state=42)
    return train_data, val_data
